...
router.patch('/me/profile', authenticate, validateProfileUpdate, async (req, res) => {
  try {
    ...
    const updates = {};
    const { displayName, timezone, preferences, notifications, phoneNumber, dateOfBirth, studyLevel, institution, bio } = req.body;
...
router.patch('/me/privacy', authenticate, validatePrivacyUpdate, async (req, res) => {
  try {
    const updates = {};
    const { cameraConsent, guardianSharing, shareFields, notifications } = req.body;
...
router.post('/me/guardian-invite', authenticate, sensitiveLimiter, validateGuardianInvite, async (req, res) => {
  try {
    const { guardianEmail, relationship, shareFields, accessLevel = 'view' } = req.body;
