...
router.get('/sessions', authenticate, async (req, res) => {
  try {
    ...
    const completedSessions = sessions.filter(s => s.status === 'completed' || (s.endTime && !s.status)).length;
...
router.get('/user-stats', authenticate, async (req, res) => {
  try {
    ...
    // Get all completed sessions (status=completed or sessions with endTime but no status)
    const allSessions = await Session.find({
      userId,
      $or: [
        { status: 'completed' },
        { endTime: { $ne: null }, status: { $exists: false } }
      ]
    }).lean();
