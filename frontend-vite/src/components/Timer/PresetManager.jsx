...
const PresetManager = ({ isOpen, onClose, onPresetsChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPreset) {
        ...
        const response = await api.presets.update(presetId, presetData)
        if (response.success) {
      ...
      if (savedPreset) {
        if (savedPreset._id && !savedPreset.id) {
          savedPreset.id = savedPreset._id;
        } else if (savedPreset.id && !savedPreset._id) {
          savedPreset._id = savedPreset.id;
        }
      }
      
      if (editingPreset) {
        const editingId = editingPreset._id || editingPreset.id;
        const index = localPresets.findIndex((p) => (p._id || p.id) === editingId)
        if (index >= 0) {
      ...
      console.log('✅ Preset saved to localStorage. Total presets:', localPresets.length)

      await loadPresets()
      if (onPresetsChange) {
        console.log('🔄 Calling onPresetsChange to reload parent component')
        onPresetsChange()
      }
      resetForm()
    } catch (error) {
  ...
  const handleDelete = async (presetId) => {
    ...
    try {
      ...
      const localPresets = JSON.parse(localStorage.getItem('timerPresets') || '[]')
      const updatedPresets = localPresets.filter((p) => (p._id || p.id) !== presetId)
