...
const Resources = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingResource) {
        const response = await api.resources.update(editingResource.id, data)
        if (response.success) {
  ...
  const handleLaunch = async (resource) => {
    try {
      await api.resources.launch(resource.id)
  ...
  const toggleFavorite = async (resource) => {
    try {
      const response = await api.resources.update(resource.id, {
        ...resource,
