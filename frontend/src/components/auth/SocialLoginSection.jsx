...
const SocialLoginSection = ({ className = '', mode = 'signin' }) => {
  ...
  console.log('SocialLoginSection rendering, mode:', mode);
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="space-y-2">
        {/* Google OAuth Button - Different routes for signin/signup */}
        <button
          onClick={() => {
            console.log('Google button clicked, mode:', mode);
            const oauthUrl = mode === 'signup' 
              ? 'http://localhost:5004/api/auth/google/signup'
              : 'http://localhost:5004/api/auth/google/signin';
